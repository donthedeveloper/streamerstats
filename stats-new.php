...
function buildStatisticsOutput($numberOfStreams) {
  ...
  $statistics .= "    <span class='column__number stats-highlight'>" . $numberOfStreams . "</span>";
...
if ($_GET['streamer']) {
  ...
  if ($validatedEmptyInput['streamer']) {
    ...
    if ( $validateInput->isAlphaNumeric($_GET['streamer']) ) {
      if ( $db->streamerDoesNotExistInDb() ) {
        ...
        if ( $db->streamerExistsOnTwitch() ) {
          ...
          $error = "We were unable to add the streamer name to the database.";
        }
      }
      else {
...
?>
    ...
    <script type="text/javascript">
      var calculationsToRender = <?php echo json_encode($renderArray) ?>;
    </script>
